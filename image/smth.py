...
model=(BirdResnet())
model.load_state_dict(torch.load('/home/venkat1924/aimlEL/image/weights/bird-resnet34best.pth',map_location=torch.device(device)))
...
def predict_image(path):
    ...
    return (bird_name_map.get(preds[0].item()).title(),round(torch.max(yb).item(), 4)*100 )
    print('Predicted:',bird_name_map.get(preds[0].item()),' with a probability of',str(round(torch.max(yb).item(), 4)*100)+'%')

print(predict_image('/home/venkat1924/aimlEL/image/test/glossyibis.jpeg')[0])
